...
`--http2` still works and multiplexes each client's streams over one
connection per gateway, but with the h2 crate at 0.4.19 (August 2026) a
client kills a connection after 100 zero-length DATA frames, and every
...
chunks, measure over HTTP/1.1; the h1 pool is sized (4096 idle per
host) so the generator is not the bottleneck.
