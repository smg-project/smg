...
ARM_B_WORKER="${TAU2_ARM_B_WORKER:-vllm}"
SGLANG_EXTRA="${TAU2_SGLANG_EXTRA:-}"
...
free_low_port() {
  ...
  echo "[launch_arms] no free port <=55000 after 20 tries" >&2
  return 1
}
...
case "$ARM" in
  ...
  b)
    case "$ARM_B_WORKER" in
      ...
      *) echo "[launch_arms] unsupported arm-B worker: $ARM_B_WORKER" >&2; exit 2 ;;
    esac
    ...
    wait_http "http://127.0.0.1:$ARM_B_GW_PORT/readiness" "${TAU2_STARTUP_TIMEOUT:-420}"
    kill "$log_tail" 2>/dev/null || true
