...
- GPU: one NVIDIA B200 in a Kubernetes pod
...
Worker, and the engine frontend, excluding the TokenSpeed scheduler; it comes
from the runner's per-process CPU report, which the committed JSON does not
include (the JSON carries the cgroup totals only).
