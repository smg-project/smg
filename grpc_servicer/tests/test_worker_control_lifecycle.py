...
def test_constructor_rejection_is_a_startup_failure(fake_smg):
    """What the extension refuses (e.g. inference on sglang) propagates
    unchanged; nothing downgrades to a servicer without a control plane."""
    fake_smg.WorkerControlServer = RejectingServer

    with pytest.raises(ValueError, match="engine_type sglang cannot serve inference"):
        WorkerControlLifecycle.start_from_env(
            engine_type="sglang",
            model_ids=["model-a"],
            features=["generate", "abort"],
...
def test_missing_extension_is_a_startup_failure(monkeypatch):
    monkeypatch.setitem(sys.modules, "smg.worker", None)

    with pytest.raises(RuntimeError, match="SMG Python extension is required"):
        WorkerControlLifecycle.start_from_env(
            engine_type="vllm",
            model_ids=["model-a"],
            features=["generate"],
