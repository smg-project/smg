...
async def serve_grpc(server_args: ServerArgs) -> None:
    ...
    advertised_model = getattr(server_args, "served_model_name", None) or model_path
    model_ids = (
    ...
    try:
        worker_control = WorkerControlLifecycle.start_from_env(
            ...
            # The Router resolves the tokenizer from these, so they name the
            # model files, never the served alias.
            engine_attributes={
