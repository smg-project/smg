...
The extension import is lazy: engine servicers that do not set
``SMG_WORKER_CONTROL_BIND_ADDRESS`` never import it. Once enabled, the control
plane is fail-closed: a missing extension or a configuration the extension
rejects propagates to the servicer's startup-failure path.
"""
