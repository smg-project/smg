...
class _TestToolChoiceBase:
    ...
    def _assert_auto_tool_calls_valid(self, tool_calls, tools, *, require_declared_name=True):
        """Every produced tool call must have JSON-dict arguments.

        ``require_declared_name`` also pins that the name is one of the
        declared tools. It is relaxed for FLAKY_TESTS models because on this
        path the name is a *model* property, not a gateway one: the
        non-streaming parser entry point takes no tool list at all
        (``ToolParser::parse_complete(&self, text)``), so an undeclared name
        cannot be filtered out, and a weak tool-caller happily invents one -
        Llama-3.2-1B answers "What is 2+2?" with a tool call literally named
        ``2+2``. The streaming path does receive the tool list and rejects
        undeclared names (they surface as content), so its validator keeps the
    ...
    def test_tool_choice_auto_non_streaming(self, model, api_client):
        ...
        *decision* (call vs. answer) is relaxed but every produced tool call
        ...
        if flaky and choice.message.tool_calls:
            ...
            self._assert_auto_tool_calls_valid(
                choice.message.tool_calls, tools, require_declared_name=False
            )
            assert choice.finish_reason == "tool_calls", (
                f"Expected finish_reason 'tool_calls', got {choice.finish_reason!r}"
            )
