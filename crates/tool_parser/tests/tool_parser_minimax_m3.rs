...
#[tokio::test]
async fn test_m3_streaming_eof_returns_unannounced_candidates_and_resets_state() {
    let tools = create_test_tools();
    let wrapper = format!("{NS}<tool_call>");
    let candidates = [
...
#[tokio::test]
async fn test_m3_streaming_eof_drops_announced_invoke_markup() {
    let tools = create_test_tools();
    let mut parser = MinimaxM3Parser::new();
    let incomplete = format!("{NS}<tool_call>\n{NS}<invoke name=\"get_weather\">");
    ...
    let pending = parser.parse_incremental(&incomplete, &tools).await.unwrap();
    assert_eq!(
        pending.calls.iter().find_map(|item| item.name.as_deref()),
        Some("get_weather")
    );
    assert_eq!(parser.take_unstreamed_normal_text(), "");
