...
impl ModelProcessorSpec for DeepseekV4Spec {
    ...
    fn prompt_replacements(
        ...
        // Surplus `types` entries mean the concatenated data does not match
        // the declared per-image lengths; fail instead of silently dropping.
        if remaining != 0 {
            return Err(ModelRegistryError::InvalidPreprocessedField {
                field: "types".to_string(),
            });
...
mod tests {
    ...
    #[test]
    fn prompt_replacements_rejects_surplus_types() {
        let tokenizer = TestTokenizer::new(&[(IMAGE_PLACEHOLDER, 129264)]);
        let config = json!({"vocab_size": 129280});
        let metadata = ModelMetadata {
            model_id: "deepseek-v4-vision",
            tokenizer: &tokenizer,
        ...
        // One declared block of 4 types, but the flat tensor carries 6.
        let preprocessed = PreprocessedEncoderInputs::new(
            ndarray::ArrayD::zeros(ndarray::IxDyn(&[4, 3, 14, 14])),
            vec![4],
            vec![(56, 56)],
        )
        .with_extra(
            ...
            ModelSpecificValue::IntTensor {
                data: vec![1, 1, 1, 0, 2, 2],
                shape: vec![6],
            },
        )
        .with_extra("types_lengths", ModelSpecificValue::IntVec(vec![4]));
        assert!(matches!(
            DeepseekV4Spec.prompt_replacements(&metadata, &preprocessed),
            Err(ModelRegistryError::InvalidPreprocessedField { .. })
        ));
