...
/// An unspecified bind address (`0.0.0.0` or `::`) is not dialable; advertise
/// the loopback of the same family instead.
fn advertised_address(bound: SocketAddr) -> SocketAddr {
...
impl MockWorkerControl {
    /// `bound` is the address the worker's gRPC listener is bound on.
    pub fn new(config: &Config, bound: SocketAddr) -> Self {
        let advertised = advertised_address(bound);
        let worker_id = format!("mock-worker-{}", advertised.port());
        ...
        Self {
            state: Arc::new(State {
                identity: proto::WorkerIdentity {
                    ...
                    hostname: advertised.ip().to_string(),
                    zone: String::new(),
                ...
                topology: proto::WorkerTopology {
                    ...
                    engines: vec![proto::EngineEndpoint {
                        ...
                        engine_type: "tokenspeed".to_string(),
                        // `SocketAddr` brackets IPv6 hosts.
                        endpoint: format!("grpc://{advertised}"),
                        model_ids: vec![config.model_id.clone()],
...
mod tests {
    ...
    #[test]
    fn unspecified_bind_addresses_are_advertised_as_dialable_loopback() {
        for (bound, hostname, endpoint) in [
            ("0.0.0.0:19000", "127.0.0.1", "grpc://127.0.0.1:19000"),
            ("[::]:19000", "::1", "grpc://[::1]:19000"),
            ("[::1]:19001", "::1", "grpc://[::1]:19001"),
            ("10.0.0.5:19002", "10.0.0.5", "grpc://10.0.0.5:19002"),
        ] {
            let control = MockWorkerControl::new(&config(), bound.parse().unwrap());
            assert_eq!(control.state.identity.hostname, hostname, "{bound}");
            assert_eq!(
                control.state.topology.engines[0].endpoint, endpoint,
                "{bound}"
            );
        }
    }
    ...
    async fn discovery_contract_works_over_a_real_tonic_channel() {
        ...
        let control = MockWorkerControl::new(&config(), address);
        ...
        assert_eq!(
            (capabilities.api_major, capabilities.api_minor),
            (WORKER_CONTROL_API_MAJOR, WORKER_CONTROL_API_MINOR)
        );
        ...
        assert_eq!(topology.engines[0].endpoint, format!("grpc://{address}"));
        servers.abort_all();
    }
}
