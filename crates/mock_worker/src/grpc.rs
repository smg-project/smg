...
pub async fn serve_with_listener(cfg: Arc<Config>, listener: TcpListener) {
    let addr = match listener.local_addr() {
        Ok(addr) => addr,
        Err(e) => {
            tracing::error!("grpc worker failed to read its bound address: {e}");
            return;
        }
    };
    // One simulated engine per listener (i.e. per virtual worker).
    let engine = cfg.realistic.then(|| Engine::spawn(cfg.engine.clone()));
    // The control plane advertises the bound address, which for a port-0
    // listener is only known now.
    let control = MockWorkerControl::new(&cfg, addr);
...
#[cfg(test)]
mod tests {
    use smg_grpc_client::worker_inference::{EngineTransport, TokenSpeedWorkerInference};
    use tokio_stream::wrappers::TcpListenerStream;
    ...
    /// TokenSpeed chunks are already deltas: every chunk must reach the
    /// `WorkerInference` wire unchanged, and only `Complete` is cumulative.
    #[tokio::test]
    async fn tokenspeed_adapter_passes_delta_chunks_through_unchanged() {
        ...
        let mut stream = adapter.generate(request).await.unwrap();
