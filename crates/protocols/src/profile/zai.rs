...
//!
//! The GLM-5.3 series (GLM-5-Next with it, as the multimodal registry has
//! it) cannot switch thinking off and takes efforts low/high/max; other GLM
//! generations keep their thinking switch and the documented wider set. The
...
//! `max_tokens` beyond the window is the gateway's (`enforces_output_budget`).
...
/// The GLM-5.3 series, the one that cannot switch thinking off; the same
/// family the multimodal registry matches (`registry/glm53_flash.rs`).
const GLM53_MARKERS: [&str; 8] = [
    "glm-5.3",
    "glm5.3",
    "glm_5.3",
    "glm-5-3",
...
mod tests {
    ...
    fn glm53_thinking_rules_do_not_reach_other_generations() {
        ...
        // The multimodal registry treats GLM-5-Next and dash-spelled ids as the
        // same family (registry/glm53_flash.rs); the profile matches the same.
        for model in [
            "GLM-5.3-Flash",
            "glm-5.3",
            "zai-org/glm5.3-air",
