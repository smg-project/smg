...
/// `f32` in its shortest round-trip form, so encode with it and parse that
/// back. Only typed `f32` fields are exact this way: numbers kept as
/// `Value` (an `other` map, nested values) were normalized at parse time
