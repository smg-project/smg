...
use crate::{
    ...
    worker::{ExternalWorker, SelectWorkerRequest, WorkerSource},
    ExternalContext, ExternalRouter, RetryConfig,
};
...
pub struct OpenAIRouter {
    ...
    webrtc_bind_addr: Option<IpAddr>,
    webrtc_stun_server: Option<String>,
}

impl std::fmt::Debug for OpenAIRouter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let registry_stats = self.workers.stats();
...
impl OpenAIRouter {
    ...
    pub async fn new(ctx: &ExternalContext) -> Result<Self, String> {
        let mcp_orchestrator = ctx.mcp_orchestrator("OpenAI")?;
        ...
        let responses_components = Arc::new(ResponsesComponents {
            ...
            response_storage: ctx.responses.clone(),
            conversation_storage: ctx.conversations.clone(),
            conversation_item_storage: ctx.conversation_items.clone(),
        });
        ...
        Ok(Self {
            ...
            retry_config: ctx.retry.clone(),
            realtime_registry: ctx.realtime.clone(),
...
impl ExternalRouter for OpenAIRouter {
    fn as_any(&self) -> &dyn Any {
        self
    }

    async fn health_generate(&self, _req: Request<Body>) -> Response {
        health::health_generate(self.workers.as_ref())
    }

    async fn get_server_info(&self, _req: Request<Body>) -> Response {
        health::get_server_info(self.workers.as_ref())
    }
    ...
    async fn route_chat(
        ...
        let per_model_retry_config = self.workers.retry_config(model_id);
    ...
    async fn route_realtime_webrtc(&self, req: Request<Body>, model: &str) -> Response {
        ...
        let body = match axum::body::to_bytes(body, WEBRTC_REQUEST_BODY_LIMIT).await {
            ...
            Err(e) => {
                ...
                return crate::error::bad_request(
                    "invalid_body",
