...
fn translate_sampling(
    ...
    // A request-supplied primary EOS wins over this client's own lookup: a
    // two-tier Worker has no tokenizer, and an id that only rides
    // `stop_token_ids` finishes as `matched_stop` rather than a plain EOS.
