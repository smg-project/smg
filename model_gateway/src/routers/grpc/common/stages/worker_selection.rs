...
impl PipelineStage for WorkerSelectionStage {
    async fn execute(&self, ctx: &mut RequestContext) -> Result<(), Response> {
        ...
        // A multimodal payload, processed here or by the prefill leg, needs a
        // vision-capable decode worker; retries re-select under the same pin.
        let multimodal = intermediate.is_some() || media_refs;
        ctx.state.multimodal_payload = multimodal;
...
/// Candidate predicate for requests carrying a multimodal payload: a
/// `--language-model-only` decode worker cannot take their decode leg.
