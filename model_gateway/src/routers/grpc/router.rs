...
impl GrpcRouter {
    ...
    /// runs ahead of worker selection. Tokenizer lookup also rejects unknown
    /// models, but checking here avoids preparation work and does not depend
    /// on whether a tokenizer is already cached. Alias-safe: `contains_model`
...
mod pd_tests {
    ...
    /// This preserves the 404 contract independently of tokenizer lookup. Regular
    /// mode is the one mode that serves all seven (embeddings and classify are
    /// single-worker only).
    #[tokio::test]
    async fn unknown_model_is_rejected_before_the_pipeline_on_every_endpoint() {
        ...
        let responses: Vec<(&str, Response)> = vec![
            (
                "responses",
                router
                    .route_responses(None, &tenant_meta, responses_request(model), model)
                    .await,
            ),
            (
                "chat",
        ...
        for (endpoint, response) in responses {
            ...
            assert_eq!(body["error"]["code"], "model_not_found", "{endpoint}");
        }
    }

    /// Registered models and their aliases must retain the server-error
    /// contract while their tokenizer is still loading.
    #[tokio::test]
    async fn served_model_without_tokenizer_is_still_a_server_error() {
        let ctx = grpc_ctx(regular_routing_mode()).await;
        ...
        for model in ["canonical-model", "model-alias"] {
            let response = router
                .route_chat(
                    None,
                    &tenant_meta,
            ...
            let body = axum::body::to_bytes(response.into_body(), usize::MAX)
                ...
                .expect("error body");
            let body: serde_json::Value = serde_json::from_slice(&body).expect("JSON error");
            assert_eq!(body["error"]["code"], "tokenizer_not_found", "{model}");
        }
    }
    ...
    async fn generate_wildcard_model_is_not_rejected_as_unknown() {
        ...
        // Let preparation succeed so the result distinguishes the entry-point
        // guard from tokenizer lookup (which also returns 404 on a miss).
        ctx.tokenizer_registry
            .load("wildcard-tokenizer", UNKNOWN_MODEL_ID, "test", || async {
                Ok(Arc::new(llm_tokenizer::MockTokenizer::new())
                    as Arc<dyn llm_tokenizer::traits::Tokenizer>)
            })
            .await
            .expect("wildcard tokenizer");

        // A model-less body deserializes with the wildcard. Omit the input
        // too: the preparation-stage error proves we passed the model guard
        // without needing to connect to the fixture's worker.
        let body: GenerateRequest = json_request(json!({}));
        ...
        let body = axum::body::to_bytes(response.into_body(), usize::MAX)
            ...
            .expect("error body");
        let body: serde_json::Value = serde_json::from_slice(&body).expect("JSON error");
        assert_eq!(body["error"]["code"], "resolve_input_failed");
